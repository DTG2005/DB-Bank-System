...
import { QueryResult, FieldPacket } from 'mysql2'; // Adjust import as necessary
...
type TransactionDetails = TransferDetails | LoanDisbursementDetails | PaymentDetails;

// Main transaction handling function
async function handleTransaction(transactionType: TransactionType, details: TransactionDetails): Promise<void> {
    ...
    try {
      ...
      switch (transactionType) {
        case TransactionType.TRANSFER:
          ...
          const [senderBalanceResult]: [AccountBalance[], FieldPacket[]] = await connection.query<AccountBalance[]>(
            "SELECT balance FROM accounts WHERE account_id = ?",
            [accountFrom]
          );
  
          // Validate the response
          if (!Array.isArray(senderBalanceResult) || senderBalanceResult.length === 0) {
          ...
          console.log(`Transferred ${amount} from ${accountFrom} to ${accountTo}`);
          break;
